...
✅ View User Roles - TestRoleManagementRBAC (10 tests):
  ...
  - Tenant Admin within-tenant user → 200 OK
  - Tenant Admin cross-tenant user → 403 Forbidden
...
class TestRoleManagementRBAC:
    ...
    @allure.title("Test Tenant Admin role scope: within-tenant → 200, cross-tenant → 403")
    @pytest.mark.parametrize("user_id_attr,case_label,expected_status", [
        ("TENANT_TEST_USER_ID", "within-tenant user", 200),
        ("TEST_USER_ID",        "cross-tenant user",  403),
    ])
    def test_get_user_roles_tenant_admin_scoped(
        self, tenant_admin_client, user_id_attr, case_label, expected_status
    ):
        """
        Verify Tenant Admin RBAC boundary for GET /auth/roles/user/{user_id}:
          - within-tenant user  → 200 OK
          - cross-tenant user   → 403 Forbidden

        TENANT_TEST_USER_ID: a user belonging to the Tenant Admin's own tenant
        TEST_USER_ID:        a user from a different tenant
        """
        ...
        assert response.status_code == expected_status, (
            f"Tenant Admin + {case_label}: expected {expected_status}, "
        ...
        if response.status_code == 200:
            data = response.json()
            assert "success" in data and data["success"] is True
            assert "data" in data and "roles" in data["data"]
            print(f"✓ Tenant Admin can view {case_label} roles: {data['data']['roles']}")
        else:
            print(f"✓ Tenant Admin correctly got {response.status_code} for {case_label}")
